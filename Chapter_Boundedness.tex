...
Given \autoref{Q1}, it is natural to ask what can be gleaned from \autoref{Main_Bound} about globally $F$-regular varieties of the type described in \autoref{Q1}. Unfortunately the answer is very little, while every globally $F$-regular variety is $F$-split and if $X$ is of $\epsilon$-log Fano type it is also of $\epsilon$-LCY type, we cannot sensibly ensure that the resulting $\epsilon$-LCY pair $(X,\Delta)$ has coefficients bounded below, even if we require it for the pair $\epsilon$-log Fano pair $(X,\Delta')$.\\
