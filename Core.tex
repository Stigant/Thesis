...
%		\item Given a morphism $f\colon X \to Y$ often write $H^{0}(X,L)$ for the $\ox{Y}$ module $f_{*}\ox$, especially if $Y$ is affine.
	...
	Together these are sufficient to prove the MMP for threefolds in positive characteristic, at least for $p > 5$. This is done in \cite{HX15},\cite{Bir16}, \cite{BW17}. It was the state of the art at the beginning of my PhD. \autoref{Boundedness} therefore focuses on some of the consequences of the MMP in this setting.
	...
	\begin{theorem*}\cite[Theorem 1.2]{witaszek2020keels}
		Let $D$ be a nef line bundle on $X$ with $D|_{X_{\mathbb{Q}}}$ semiample. Let $\mathbb{E}[D]$ be the union of integral subschemes of $X$ on which $D$ is not big. Then $D$ is semiample (resp. EWM) if and only if $D|_{\mathbb{E}[D]}$ is so.
	\end{theorem*}
