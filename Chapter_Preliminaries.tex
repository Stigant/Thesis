...
A useful application of DLT modifications is the study of the NKLT locus. In particular we have following generalisation of the Cone Theorem as well as a connectedness result for suitable pairs.
...
\begin{theorem}[Weak Connectedness Lemma]\label{WLC}
	Let $(X,\Delta)/T$ be an $R$ pair with geometrically connected fibres such that the coefficients of $\Delta$ bounded above by $1$. Then if $-(K_{X}+\Delta)$ is big and nef, $\text{Nklt}(X,\Delta)$ is either empty or connected.
\end{theorem}
