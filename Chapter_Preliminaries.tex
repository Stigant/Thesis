...
\begin{theorem}
	Let $(X,\Delta)$ an $R$ pair with coefficients bounded above by $1$. Then there is a birational morphism $f:Y \to X$, called a dlt modification, such that the following holds:
	\begin{itemize}
	...
	\end{itemize}
	Here $f(F)=\textup{Nlc}(X,\Delta)$ and $f_{*}F=\Delta-\Delta'$. Moreover $(X,\Delta)$ is dlt if and only if this is a small morphism.
\end{theorem}
...
%	and $f^{-1}_{*}\Delta$ is not supported on $E$. If $E$ is not exceptional, then we may assume $\text{Coeff}_{f_{*}E}\Delta \geq 1$. Thus $\text{Coeff}_{E}F'=\text{Coeff}_{f_{*}E}\Delta - \text{Coeff}_{f_{*}E}\Delta'=-(a(E,X,\Delta)+1)$ as claimed. 
	...
	We may choose $H,H_{s}$ so that $(Y', (1-rs)S+(1-t)F+rH_{s}+\pi_{*}^{-1}\Delta_{<1})$ is rlt for small $r,s,t > 0$ by \autoref{}. We may choose $t$ sufficiently small that $a(E,X,\Delta) > t-1$ for each $E$ in the support of $F$. Write $\pi^{*}(K_{X}+\Delta)=K_{Y'}+B$, and then choose $D$ as follows.
	\begin{align*}
	-D&=K_{Y}+S+(1-t)F+f_{*}^{-1}\Delta_{<1}+rH-\pi^{*}(K_{X}+\Delta+A)\\
	&=S+(1-t)F+f_{*}^{-1}\Delta_{<1}-rC-B
	\end{align*}
	Let $f:Y \to X$ be a minimal model for $(Y', (1-rs)S+(1-t)F+rH_{s}+\pi_{*}^{-1}\Delta_{<1})$. By construction $Y$ is $\mathbb{Q}$-factorial and is also a minimal model for the pair $(Y',S+(1-t)F+\pi_{*}^{-1}\Delta_{<1}+H)$. In particular, letting $S',F',H', H'_{s},D'$ be the strict transforms of the corresponding divisors on $Y'$, we have that $(Y,S'+(1-t)F'+f_{*}^{-1}\Delta_{<1})$ is dlt and $N=K_{Y}+S'+(1-t)F'+f_{*}^{-1}\Delta_{<1}+rH'$ is nef over $X$.
...
A useful application of DLT modifications is the study of the non-klt and non-lc loci. In particular we have following generalisation of the Cone Theorem as well as a connectedness result for suitable pairs.
	...
	Let $(X,B)/T$ be an $R$-pair with coefficients bounded above by $1$. Then write $\overline{NE}(X/T)_{nlc}$ for the cone spanned by curves contained in the non log canonical locus of $X$. Then we have the following decomposition
