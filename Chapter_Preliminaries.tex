...
\begin{itemize}
	...
	& \ox &                                     
	\end{tikzcd}\]
	\item $\mathcal{L}=\mathcal{L}^{p^{e'}+1}$ and $\phi'\colon \Fe[e+e']\mathcal{L}^{p^{e'}+1}\to \ox$ is the precisely the map given by
...
\begin{definition}\label{Model-defs}
	...
	If in fact $\phi$ is $K_{X}+\Delta$ negative, $Y$ is $\mathbb{Q}$-factorial, and $(Y,\Delta_{Y})$ is dlt then $\phi$ is a log terminal model. Again if $(X,\Delta)$ is dlt then the dlt condition on $(Y,\Delta_{Y})$ is automatic as $\phi$ is negative. If $K_{Y}+\Delta_{Y}$ is semiample then $\phi$ is said to be a good log terminal model.
