...
%	If $C$ contains only klt boundaries then $A$ is big suffices.
	...
	In this section we study termination for threefold pairs over positive dimensional bases. In this setting we will show that every $K_{X}+\Delta$ MMP terminates for a dlt pair $(X,\Delta)/T$. As always in this chapter, we consider only positive dimensional bases.
 ...
 We rely heavily on \autoref{partial-termination}. The key remaining argument is if $X \to T$ is a klt pair then there is an open set on which every contraction is a horizontal divisorial contraction. We prove this by reducing to the case that $(X,\Delta)$ is terminal. In mixed and positive characteristic this then follows from the liftablity of $-1$ curves, see \cite{KU}. This argument does not work in purely positive characteristic but provides motivation for our approach. Instead we adapt a termination argument for terminal pairs, largely due to Shokurov \cite{shokurov1986nonvanishing}.
...
\begin{theorem}[MMP for rlt pairs]\label{rltmmp}
	Let $(X,\Delta)/T$ be a threefold rlt $R$-pair with $\mathbb{R}$-boundary, then we can run a $K_{X}+\Delta$ MMP. If $K_{X}+\Delta$ is pseudo-effective then this terminates with a log terminal model, otherwise it ends in a Mori fibre space. If the output $X \dashrightarrow Y$ is a log terminal model then $K_{Y}+\Delta_{Y}$ is semiample.
\end{theorem}


\begin{proof}
	
	Existence of the claimed MMPs and their termination is immediate from the above results. Suppose then $\phi:X \dashrightarrow Y$ is a log terminal model, since semiampleness is checked locally over the base we can assume that $(X,\Delta)$ is klt. Then $K_{Y}+\Delta_{Y}$ is a good log minimal model by \autoref{Main_Abund1}.
...
%	In this section $A$ is always an ample $\mathbb{Q}$-divisor on $X$ unless otherwise specified and $V$ is a coefficient space. All the results about rlt polytopes in $\mathcal{RL}_{A}(C)$ extend to the case that $A$ is big and nef, by replacing it with $A'+E$ where $A$ is ample and $E$ is suitably small. This is not true for results about general rational (i.e. rlc) polytopes however. \myworries{Is this true?}
	...
	\begin{theorem}\cite[Theorem 9.33]{bhatt2020}
		Suppose that $X$ is $\mathbb{Q}$-factorial and let $C$ be a klt polytope in $\mathcal{L}_{A}(V)$ for $A$ big. There is a finite collection of log terminal models $\phi_{i}: X \dashrightarrow Y_{i}$ such that  every $B \in \mathcal{E}(C)$ has some $j$ with $\phi_{j}$ a log terminal model of $(X,B)$. 
	\end{theorem}
	...
	\begin{lemma}\cite[Lemma 3.6]{hacon2009sarkisov}\label{amp-geo}
		Let $(X,\Delta)/T$ be an rlt pair. Suppose that $B-\Delta$ is ample and $f\colon X \dashrightarrow Y$ is an ample model for $K_{X}+B$. If $Y$ is $\mathbb{Q}$-factorial, then $f$ is a log terminal model for $(X,\Delta)$.
	\end{lemma}
